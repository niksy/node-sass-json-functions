...
function parseValueToStringOrNumber(value, options) {
	...
	try {
		...
		if (unitTypes.includes(type)) {
			resolvedValue = new SassNumber(parsedValue, unit);
		} else if (type === 'percentage') {
			resolvedValue = new SassNumber(parsedValue, '%');
		} else if (forceNumber && !Number.isNaN(Number(value))) {
			resolvedValue = new SassNumber(Number(value));
		} else {
			resolvedValue = new SassString(value);
		}
	} catch (error) {
		resolvedValue = new SassString(value);
	}
...
function setJsonValueToSassValue(value) {
	...
	if (Array.isArray(value)) {
		...
		resolvedValue = new SassNumber(value);
	} else if (typeof value === 'boolean') {
		resolvedValue = value ? sassTrue : sassFalse;
	} else {
		resolvedValue = sassNull;
	}
	return resolvedValue;
}
...
function objectToMap(object) {
	...
	return new SassMap(OrderedMap(data));
}
