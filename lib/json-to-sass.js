...
function createSetJsonValueToSassValue(sass) {
	 ...
	 * @property {?import('sass').CalculationOperator} operator
	 * @property {?import('sass').CalculationValue}    left
	 * @property {?import('sass').CalculationValue}    right
	 */
		...
		if (node?.type === 'func' && node.name === 'calc') {
			...
			node.nodes.forEach((node, index) => {
				if (node.type === 'operator') {
					calc.operator = /** @type {import('sass').CalculationOperator} */ (node.value);
				} else if (index === 0) {
	...
	function objectToMap(object) {
		/** @type {[import('sass').Value, import('sass').Value][]} */
		const data = [];
